...
void SysTest(){
  ...
  for(size_t i_f=0;i_f<label_v.size();i_f++){
    ...
    std::string plot_dir = "Analysis/"+label+"/Plots/SysTest/Reco/";
    gSystem->Exec(("mkdir -p "+plot_dir).c_str());

    std::string sub = "Cov/Reco/Total/"; 

    // Print total covariance and total fractional covariance
    TH2D* h_Cov = static_cast<TH2D*>(f_in_hist->Get((sub+"Cov_Tot").c_str()));
    pfs::Draw2DHist(h_Cov,plot_dir+"Cov.png");

    TH2D* h_FCov = static_cast<TH2D*>(f_in_hist->Get((sub+"FCov_Tot").c_str()));
    ...
    TH2D* h_FCov_MCStat = static_cast<TH2D*>(f_in_hist->Get("Cov/Reco/MCStat/FCov_Tot"));
    pfs::Draw2DHist(h_FCov_MCStat,plot_dir+"FCov_MCStat.png");

    TH2D* h_Cov_EstDataStat = static_cast<TH2D*>(f_in_hist->Get("Cov/Reco/EstDataStat/Cov_Tot"));
    pfs::Draw2DHist(h_Cov_EstDataStat,plot_dir+"Cov_EstDataStat.png");

    TH2D* h_FCov_EstDataStat = static_cast<TH2D*>(f_in_hist->Get("Cov/Reco/EstDataStat/FCov_Tot"));
    ...
    for(int i_s=0;i_s<kSystMAX;i_s++){
      sub = "Cov/Reco/"+sys_str.at(i_s)+"/"; 
      h_FE.push_back((TH1D*)f_in_hist->Get("CV/Reco/h_Tot")->Clone(("h_FE_"+sys_str.at(i_s)).c_str()));
      colors.push_back(sys_color.at(i_s));
      legs.push_back(sys_str.at(i_s));
      TH2D* h = static_cast<TH2D*>(f_in_hist->Get((sub+"FCov_Tot").c_str()));
    ...
    h_FE.push_back((TH1D*)f_in_hist->Get("CV/Reco/h_Tot")->Clone("h_FE_EstDataStat"));
    ...
    h_FE.push_back((TH1D*)f_in_hist->Get("CV/Reco/h_Tot")->Clone("h_FE_Total"));

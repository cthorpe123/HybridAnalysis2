...
void CalcE(){
  ...
  for(int ievent=0;ievent<t_in->GetEntries();ievent++){
    ...
    std::vector<bool> cont_muon_v = {muon_contained_pd,muon_contained_wc,muon_contained_lt};
    std::vector<double> W_v = {W_pd,W_wc,W_lt};    
    std::vector<std::vector<double>*> e_v = {est_nu_e_pd,est_nu_e_wc,est_nu_e_lt}; 
  ...
  for(int i_e=0;i_e<ee::kMAX;i_e++){
    ...
    l->Draw();
    c->Print(("Plots/CalcE/RecoE_CheatMuon_"+ee::estimators_str.at(i_e)+".png").c_str());
    c->Clear();
    l->Clear();
  ...
  for(int i_e=0;i_e<ee::kMAX;i_e++){
    for(int i=0;i<kMethMAX;i++){
      ...
      c->Print(("Plots/CalcE/EstE_RecoE_CheatMuon_"+ee::estimators_str.at(i_e)+"_"+methods_str.at(i)+".png").c_str());
      c->Clear();
    }
  }
  ...
  for(int i_e=0;i_e<ee::kMAX;i_e++){
    ...
    l->Draw();
    c->Print(("Plots/CalcE/ErrorEstE_CheatMuon_"+ee::estimators_str.at(i_e)+".png").c_str());
    c->Clear();
    l->Clear();
  ...
  for(int i_e=0;i_e<ee::kMAX;i_e++){
    for(int i=0;i<kMethMAX;i++){
      ...
      c->Print(("Plots/CalcE/TrueE_RecoE_CheatMuon_"+ee::estimators_str.at(i_e)+"_"+methods_str.at(i)+".png").c_str());
      c->Clear();
    }
  }
  ...
  for(int i_e=0;i_e<ee::kMAX;i_e++){
    ...
    l->Draw();
    c->Print(("Plots/CalcE/ErrorTrueE_CheatMuon_"+ee::estimators_str.at(i_e)+".png").c_str());
    c->Clear();
    l->Clear();

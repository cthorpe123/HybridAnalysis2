...
void Filter(){
  ...
  TFile* f_in = nullptr;
  TTree* t_in = nullptr;
  LoadTree(in_dir+file,f_in,t_in,is_data);
  ...
  for(int ievent=0;ievent<t_in->GetEntries();ievent++){
    ...
    if(!is_data){
      ...
      has_muon_t = mc_pdg->size() && abs(mc_pdg->at(0)) == 13 && muon_mom_t.Mag() > 0.3;
    ...
    has_muon_pd = pd_muon != -1 && muon_mom_pd.Mag() > 0.3;
    ...
    has_muon_wc = wc_muon != -1 && muon_mom_wc.Mag() > 0.3;
    ...
    has_muon_lt = lt_muon != -1 && muon_mom_lt.Mag() > 0.3;

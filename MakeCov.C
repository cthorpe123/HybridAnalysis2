...
void MakeCov(){
  ...
  //  "test/Filtered_Merged_larpid_patch_smart_patch_test10_full_more.root",
  ...
  TH1D* h_CV_Tot = new TH1D("h_CV_Tot",axis_title.c_str(),nbins,low,high);
  ...
  for(int i_s=0;i_s<kSystMAX;i_s++){
    ...
    for(int i_u=0;i_u<sys_nuniv.at(i_s);i_u++){
      ...
      h_Vars_Tot.back().push_back(new TH1D(label.c_str(),axis_title.c_str(),nbins,low,high));
    }
  }
  
  for(size_t i_c=0;i_c<categories.size();i_c++){
    h_CV.push_back(new TH1D(("h_CV_"+categories.at(i_c)).c_str(),axis_title.c_str(),nbins,low,high));
    ...
    for(int i_s=0;i_s<kSystMAX;i_s++){
      ...
      for(int i_u=0;i_u<sys_nuniv.at(i_s);i_u++){
        ...
        h_Vars.back().back().push_back(new TH1D(label.c_str(),axis_title.c_str(),nbins,low,high));
      }
    }
  }

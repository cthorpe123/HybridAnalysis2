...
void Response::SetTemplate(std::string axis_title,int nbins,double low,double high)
{
...
{
  bin_edges.clear();
  for(int i=1;i<_h_tp->GetNbinsX()+2;i++) bin_edges.push_back(_h_tp->GetBinLowEdge(i));

  // Storing totals
  _h_CV_Truth = (TH1D*)_h_tp->Clone(("h_CV_Truth_"+_label).c_str());
  _h_CV_Truth->Sumw2();

  _h_CV_Joint = new TH2D(("h_CV_Joint_"+_label).c_str(),"",bin_edges.size()-1,&bin_edges[0],bin_edges.size()-1,&bin_edges[0]);
  _h_CV_Joint->Sumw2();
  ...
  for(int i_s=0;i_s<kSystMAX;i_s++){
    ...
    for(int i_u=0;i_u<sys_nuniv.at(i_s);i_u++){
      _h_Vars_Truth.back().push_back((TH1D*)_h_tp->Clone(("h_Vars_Truth_"+sys_str.at(i_s)+"_"+std::to_string(i_u)+"_"+_label).c_str()));
      _h_Vars_Joint.back().push_back(new TH2D(("h_Vars_Joint_"+sys_str.at(i_s)+"_"+std::to_string(i_u)+"_"+_label).c_str(),"",bin_edges.size()-1,&bin_edges[0],bin_edges.size()-1,&bin_edges[0]));
    }
  }
...
void Response::AddSpecialUniv(std::string name){
  ...
  _h_Special_Truth[name] = (TH1D*)_h_tp->Clone(("h_Special_Truth_"+name).c_str()); 
...
void Response::FillSpecialUniv(std::string name,double var_t,double var_r,bool sig,bool sel,double weight=1.0)
{
  ...
  _h_Special_Truth.at(name)->Fill(var_t,POT_weight*weightSpline*weight);  
...
void Response::Write()
{
  ...
  std::cout << "empty_bins.size()=" << empty_bins.size() << std::endl;
  ...
  for(int i_s=0;i_s<kSystMAX;i_s++){
    for(int i_u=0;i_u<sys_nuniv.at(i_s);i_u++){
      ...
      h_Vars_Joint_Unrolled.at(i_s).back()->Write((string(_h_Vars_Truth.at(i_s).at(i_u)->GetName())+"_Unrolled").c_str());
    }

...
void Recipe5(){
  ...
  for(const std::string& var : vars){
    ...
    std::string plot_dir = AnalysisDir()+"/"+var+"/Plots/Recipe5/";
    gSystem->Exec(("mkdir -p " + plot_dir).c_str());

    TFile* f_in = TFile::Open((AnalysisDir()+"/"+var+"/rootfiles/FFGenerators.root").c_str());
    TFile* f_out = new TFile((AnalysisDir()+"/"+var+"/rootfiles/Recipe5.root").c_str(),"RECREATE");
    ...
    for(std::string gen : generators){
      ...
      for(auto item : h_cov_diff_m){
        ...
        gSystem->Exec(("mkdir -p " + plot_dir_sys).c_str());
        pfs::Draw2DHist(item.second,plot_dir_sys+"Cov_"+item.first+"_"+gen+".png");
